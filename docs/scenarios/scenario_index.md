...
| `builder_merge_with_nonexistent_branch_returns_error` | GitRepoBuilder.merge() against nonexistent branch propagates run_git error | `GitRepoBuilder::merge("...", "nonexistent")` | faultline-fixtures | — | Merge enqueue + error path | — | domain | — | — | faultline-fixtures | P1 | faultline-fixtures | no |
| `delete_existing_file_succeeds` | FileOp::Delete removes an existing tracked file | `GitRepoBuilder + FileOp::Delete` | faultline-fixtures | — | Builder correctness | — | domain | — | — | faultline-fixtures | P2 | faultline-fixtures | no |
| `delete_missing_file_is_noop` | FileOp::Delete on a missing path is a no-op (guard branch) | `GitRepoBuilder + FileOp::Delete` | faultline-fixtures | — | Edge case | — | domain | — | — | faultline-fixtures | P2 | faultline-fixtures | no |
| `apply_file_ops_write_under_file_parent_returns_error` | FileOp::Write fails when parent path is a regular file | `GitRepoBuilder + FileOp::Write` | faultline-fixtures | — | Filesystem error path | — | domain | — | — | faultline-fixtures | P2 | faultline-fixtures | no |
| `apply_file_ops_delete_directory_returns_error` | FileOp::Delete fails when path is a directory | `GitRepoBuilder + FileOp::Delete` | faultline-fixtures | — | Filesystem error path | — | domain | — | — | faultline-fixtures | P2 | faultline-fixtures | no |
| `apply_file_ops_rename_missing_source_returns_error` | FileOp::Rename fails when source is missing | `GitRepoBuilder + FileOp::Rename` | faultline-fixtures | — | Filesystem error path | — | domain | — | — | faultline-fixtures | P2 | faultline-fixtures | no |
...
| `prepare_run_treats_foreign_pid_lock_as_stale_on_non_unix` | Foreign-PID lock treated as stale on non-Unix | Lock with PID 1 | faultline-store | — | Platform-aware lock handling | — | adapter | — | — | faultline-store | P0 | faultline-store | no |
...
| `capture_output_returns_status_stdout_and_stderr` | Process capture returns status and streams | Rust process command | xtask | command output | Runner capture contract | RIPR verification adoption | app | verification | process runner | xtask | P1 | verification/ripr | no |
| `capture_output_with_timeout_reports_completed_process` | Timeout capture reports completed child process | Rust process command | xtask | command output | Timeout completion contract | RIPR verification adoption | app | verification | process runner | xtask | P1 | verification/ripr | no |
| `capture_output_with_timeout_reports_timed_out_process` | Timeout capture reports timed-out child process | Rust process command | xtask | command output | Timeout enforcement contract | RIPR verification adoption | app | verification | process runner | xtask | P1 | verification/ripr | no |
| `check_rejects_stale_annotations` | Annotation check rejects stale generated output | TempDir + comments JSON | xtask | target/ripr/review/annotations.txt | Annotation freshness | RIPR verification adoption | app | verification | review annotations | xtask | P1 | verification/ripr | no |
| `check_rejects_stale_outputs` | PR evidence check rejects stale packet output | TempDir + PR evidence files | xtask | target/ripr/pr/repo-exposure.json | PR evidence freshness | RIPR verification adoption | app | verification | pr evidence | xtask | P1 | verification/ripr | no |
| `check_rejects_stale_summary` | PR summary check rejects stale summary output | TempDir + summary Markdown | xtask | target/ripr/pr/summary.md | Summary freshness | RIPR verification adoption | app | verification | pr summary | xtask | P1 | verification/ripr | no |
| `comments_emit_warning_annotations_only` | Review comments become warning annotations only | Review comments JSON | xtask | target/ripr/review/annotations.txt | Non-blocking annotation policy | RIPR verification adoption | app | verification | review annotations | xtask | P0 | verification/ripr | no |
| `comments_without_safe_placement_fail_contract` | Non-placeable comments fail annotation placement contract | Review comments JSON | xtask | target/ripr/review/comments.json | Changed-line placement | RIPR verification adoption | app | verification | review comments | xtask | P1 | verification/ripr | no |
| `default_ripr_binary_is_path_binary` | Default RIPR command resolves from PATH | Environment defaults | xtask | - | Tool resolution | RIPR verification adoption | app | verification | command wrapper | xtask | P2 | verification/ripr | no |
| `error_packet_is_contract_valid` | Error PR evidence packet validates against schema | Hand-built packet | xtask | target/ripr/pr/repo-exposure.json | Error packet contract | RIPR verification adoption | app | verification | pr evidence schema | xtask | P1 | verification/ripr | no |
| `explicit_full_owner_label_routes_full_owner_only` | Explicit full-owner label routes full-owner mutation only | Label inputs | xtask | target/xtask/impacted-evidence/latest.json | Mutation routing policy | RIPR verification adoption | app | verification | impacted evidence | xtask | P0 | verification/ripr | no |
| `markdown_renders_stable_summary_sections` | PR evidence Markdown includes stable sections | Hand-built evidence packet | xtask | target/ripr/pr/repo-exposure.md | Markdown summary shape | RIPR verification adoption | app | verification | pr evidence markdown | xtask | P1 | verification/ripr | no |
| `missing_comments_artifact_is_advisory_noop` | Missing review comments artifact is advisory no-op | Missing file fixture | xtask | target/ripr/review/annotations.txt | Advisory annotation behavior | RIPR verification adoption | app | verification | review annotations | xtask | P2 | verification/ripr | no |
| `missing_custom_pr_evidence_reports_custom_path` | Impacted evidence reports missing custom PR evidence path | Missing file fixture | xtask | target/xtask/impacted-evidence/latest.json | Missing artifact reporting | RIPR verification adoption | app | verification | impacted evidence | xtask | P1 | verification/ripr | no |
| `missing_pr_evidence_is_explicit_and_label_only` | Impacted evidence is explicit when PR evidence is absent | Missing file fixture + labels | xtask | target/xtask/impacted-evidence/latest.json | Label-only routing fallback | RIPR verification adoption | app | verification | impacted evidence | xtask | P1 | verification/ripr | no |
| `owned_run_helpers_report_success_and_failure_status` | Owned run helpers report command status correctly | Rust process command | xtask | command output | Runner status contract | RIPR verification adoption | app | verification | process runner | xtask | P1 | verification/ripr | no |
| `packet_maps_check_summary_to_routing_fields` | PR evidence maps RIPR check summary to routing fields | RIPR check JSON fixture | xtask | target/ripr/pr/repo-exposure.json | Routing field mapping | RIPR verification adoption | app | verification | pr evidence schema | xtask | P0 | verification/ripr | no |
| `packet_without_check_summary_is_incomplete_and_warns` | Missing RIPR check summary yields incomplete warning packet | RIPR check JSON fixture | xtask | target/ripr/pr/repo-exposure.json | Incomplete evidence warning | RIPR verification adoption | app | verification | pr evidence schema | xtask | P1 | verification/ripr | no |
| `parse_accepts_check_only` | PR evidence args accept check-only mode | CLI arg vector | xtask | - | Argument parsing | RIPR verification adoption | app | verification | command wrapper | xtask | P2 | verification/ripr | no |
| `parse_accepts_labels_and_check` | Impacted evidence args accept labels and check mode | CLI arg vector | xtask | - | Argument parsing | RIPR verification adoption | app | verification | command wrapper | xtask | P2 | verification/ripr | no |
| `parse_defaults_and_check_mode` | PR evidence args apply defaults and check mode | CLI arg vector | xtask | - | Argument defaults | RIPR verification adoption | app | verification | command wrapper | xtask | P2 | verification/ripr | no |
| `parse_rejects_unknown_or_empty_args` | Command wrappers reject unknown and empty args | CLI arg vector | xtask | - | Argument validation | RIPR verification adoption | app | verification | command wrapper | xtask | P1 | verification/ripr | no |
| `parse_supports_paths_and_check` | Review comment args support paths and check mode | CLI arg vector | xtask | - | Argument parsing | RIPR verification adoption | app | verification | command wrapper | xtask | P2 | verification/ripr | no |
| `rejects_missing_badge_field` | Badge schema rejects missing Shields endpoint fields | Badge JSON fixture | xtask | badges/ripr.json | Badge schema validation | RIPR verification adoption | app | verification | badge schema | xtask | P1 | verification/ripr | no |
| `ripr_severe_gap_routes_targeted_mutation` | Severe RIPR gap routes targeted mutation | PR evidence JSON fixture | xtask | target/xtask/impacted-evidence/latest.json | Mutation routing policy | RIPR verification adoption | app | verification | impacted evidence | xtask | P0 | verification/ripr | no |
| `run_in_dir_reports_success_and_failure_with_cwd` | Runner reports success and failure with explicit working directory | Rust process command | xtask | command output | Working-directory runner contract | RIPR verification adoption | app | verification | process runner | xtask | P1 | verification/ripr | no |
| `run_output_optional_returns_empty_for_failure` | Optional runner output returns empty string on failure | Rust process command | xtask | command output | Optional output fallback | RIPR verification adoption | app | verification | process runner | xtask | P2 | verification/ripr | no |
| `run_output_owned_includes_stderr_on_failure` | Owned output helper includes stderr on failure | Rust process command | xtask | command output | Failure diagnostics | RIPR verification adoption | app | verification | process runner | xtask | P1 | verification/ripr | no |
| `run_output_reports_stdout_and_failure` | Output helper reports stdout and failure status | Rust process command | xtask | command output | Runner output contract | RIPR verification adoption | app | verification | process runner | xtask | P1 | verification/ripr | no |
| `run_reports_success_and_failure_status` | Runner reports success and failure status | Rust process command | xtask | command output | Runner status contract | RIPR verification adoption | app | verification | process runner | xtask | P1 | verification/ripr | no |
| `summary_handles_error_packets_and_invalid_json` | PR summary handles error packets and invalid JSON | PR evidence JSON fixtures | xtask | target/ripr/pr/summary.md | Summary error handling | RIPR verification adoption | app | verification | pr summary | xtask | P1 | verification/ripr | no |
| `summary_makes_missing_artifacts_explicit` | PR summary explicitly reports missing artifacts | Missing artifact fixture | xtask | target/ripr/pr/summary.md | Missing artifact visibility | RIPR verification adoption | app | verification | pr summary | xtask | P1 | verification/ripr | no |
| `summary_renders_from_machine_readable_artifacts` | PR summary renders from machine-readable artifacts | PR evidence and comments fixtures | xtask | target/ripr/pr/summary.md | Summary source of truth | RIPR verification adoption | app | verification | pr summary | xtask | P0 | verification/ripr | no |
| `terminate_after_timeout_returns_false_for_already_finished_child` | Timeout termination returns false for finished child | Rust process command | xtask | command output | Timeout termination semantics | RIPR verification adoption | app | verification | process runner | xtask | P1 | verification/ripr | no |
| `timeout_was_enforced_requires_termination_and_unsuccessful_status` | Timeout enforcement requires termination and non-success status | Rust process command | xtask | command output | Timeout enforcement semantics | RIPR verification adoption | app | verification | process runner | xtask | P1 | verification/ripr | no |
| `validates_minimal_shields_endpoint` | Badge validation accepts minimal Shields endpoint | Badge JSON fixture | xtask | badges/ripr.json | Badge schema validation | RIPR verification adoption | app | verification | badge schema | xtask | P1 | verification/ripr | no |
| `validates_required_fields_and_types` | Contract validator enforces required fields and types | JSON schema fixtures | xtask | schemas/ripr/*.schema.json | Schema validation | RIPR verification adoption | app | verification | verification schemas | xtask | P1 | verification/ripr | no |
| `validation_rejects_changed_file_drift` | PR evidence validation rejects changed-file count drift | PR evidence JSON fixture | xtask | target/ripr/pr/repo-exposure.json | PR evidence consistency | RIPR verification adoption | app | verification | pr evidence schema | xtask | P1 | verification/ripr | no |
| `validation_rejects_summary_count_drift` | PR evidence validation rejects summary count drift | PR evidence JSON fixture | xtask | target/ripr/pr/repo-exposure.json | PR evidence consistency | RIPR verification adoption | app | verification | pr evidence schema | xtask | P1 | verification/ripr | no |
| `validation_requires_markdown_artifact` | PR evidence validation requires Markdown artifact | PR evidence fixture | xtask | target/ripr/pr/repo-exposure.md | Artifact completeness | RIPR verification adoption | app | verification | pr evidence markdown | xtask | P1 | verification/ripr | no |
| `write_and_check_packet_in_git_repo` | PR evidence writes and checks packet in a real git repo | Temp git repo | xtask | target/ripr/pr/repo-exposure.json | End-to-end PR evidence wrapper | RIPR verification adoption | app | verification | pr evidence | xtask | P0 | verification/ripr | no |
| `write_wrapper_accepts_successful_producer` | Review comment wrapper accepts successful producer output | Review comments JSON fixture | xtask | target/ripr/review/comments.json | Review wrapper happy path | RIPR verification adoption | app | verification | review comments | xtask | P1 | verification/ripr | no |
| `write_wrapper_converts_producer_failure_to_error_packet` | Review comment wrapper converts producer failure to error packet | Failing producer fixture | xtask | target/ripr/review/comments.json | Review wrapper error packet | RIPR verification adoption | app | verification | review comments | xtask | P1 | verification/ripr | no |
| `family_matches_exact_and_any` | No-panic policy: family matcher accepts exact name and `any` wildcard | Hand-built family strings | xtask | — | Family matcher correctness | — | app | — | — | xtask | P1 | xtask | no |
| `path_matches_exact_and_glob` | No-panic policy: path matcher supports both exact-path and glob entries | Hand-built allow entries | xtask | — | Path matcher correctness | — | app | — | — | xtask | P1 | xtask | no |
| `selector_baseline_glob_always_matches` | No-panic policy: `baseline_glob` selector matches every finding | Hand-built selector | xtask | — | Baseline selector semantics | — | app | — | — | xtask | P1 | xtask | no |
| `rust_and_markdown_excluded` | File policy: Rust and Markdown files are exempt from the non-Rust allowlist | Hand-built paths | xtask | — | Exemption correctness | — | app | — | — | xtask | P1 | xtask | no |
| `entry_path_and_glob_matching` | File policy: allow entries match by exact path and by glob pattern | Hand-built allow entries | xtask | — | Path matcher correctness | — | app | — | — | xtask | P1 | xtask | no |
| `ensure_passes_on_true` | Fallible test helpers: ensure! returns Ok on true predicates | Hand-built predicate | faultline-fixtures | — | Helper happy path | — | app | — | — | faultline-fixtures | P1 | testing | no |
| `ensure_fails_on_false` | Fallible test helpers: ensure! returns Err with caller message on false | Hand-built predicate | faultline-fixtures | — | Helper error path | — | app | — | — | faultline-fixtures | P1 | testing | no |
| `ensure_eq_propagates_message` | Fallible test helpers: ensure_eq! reports both sides on mismatch | Hand-built values | faultline-fixtures | — | Helper error path | — | app | — | — | faultline-fixtures | P1 | testing | no |
| `require_ok_unwraps_ok` | Fallible test helpers: require_ok! unwraps Ok results | Hand-built Result | faultline-fixtures | — | Helper happy path | — | app | — | — | faultline-fixtures | P1 | testing | no |
| `require_ok_propagates_err` | Fallible test helpers: require_ok! propagates Err with context | Hand-built Result | faultline-fixtures | — | Helper error path | — | app | — | — | faultline-fixtures | P1 | testing | no |
| `require_some_unwraps_some` | Fallible test helpers: require_some! unwraps Some values | Hand-built Option | faultline-fixtures | — | Helper happy path | — | app | — | — | faultline-fixtures | P1 | testing | no |
| `require_some_propagates_none` | Fallible test helpers: require_some! returns Err for None | Hand-built Option | faultline-fixtures | — | Helper error path | — | app | — | — | faultline-fixtures | P1 | testing | no |
