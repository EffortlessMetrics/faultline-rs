...
fn write_review_comments_with_runner(
    ...
    normalize_review_comments_packet(repo)?;
...
fn normalize_review_comments_packet(repo: &Path) -> Result<(), String> {
    ...
    if let Some(object) = packet.as_object_mut()
        && !object.contains_key("rendering_limits")
        && let Some(limits) = object.remove("limits")
    {
        object.insert("rendering_limits".to_string(), limits);
    }
    ...
    fs::write(
        ...
        format!(
            ...
            serde_json::to_string_pretty(&packet)
                .map_err(|err| format!("serialize normalized review comments: {err}"))?
        ),
    )
    .map_err(|err| format!("write normalized {REVIEW_COMMENTS_JSON}: {err}"))?;
    Ok(())
}
...
fn validate_packet_value(
    ...
    match packet.get("root").and_then(Value::as_str) {
        Some(actual) if actual == expected_root || actual == options.root => {}
        Some(actual) => violations.push(format!(
            "root is {actual:?}, expected {expected_root:?} or {:?}",
            options.root
        )),
        None => violations.push("root is missing or not a string".to_string()),
    }
