...
/// Emit a coarser per-(path, family) baseline for committing as the
/// receipted-debt floor. New panic-family files / new families in existing
/// files are NOT covered by this baseline — that is intentional, so the
/// blast radius of the catch-all `glob = "**/*"` baseline is narrowed.
pub fn baseline(root: &Path) -> Result<()> {
    println!("=== no-panic baseline ===\n");
    let findings = scan_workspace(root)?;
    let mut grouped: BTreeMap<(String, String), usize> = BTreeMap::new();
    for f in &findings {
        *grouped
            .entry((f.path.clone(), f.family.clone()))
            .or_default() += 1;
    }

    let dir = report_dir(root)?;
    let out_path = dir.join("no-panic-baseline.toml");

    use std::fmt::Write;
    let mut s = String::new();
    let _ = writeln!(s, "# Per-(file, family) baseline for the no-panic policy.");
    let _ = writeln!(
        s,
        "# Generated by `cargo xtask no-panic baseline`. Review and copy entries"
    );
    let _ = writeln!(s, "# into policy/no-panic-allowlist.toml. Burn down by:");
    let _ = writeln!(
        s,
        "#   1. converting an entry's surface to fallible alternatives,"
    );
    let _ = writeln!(
        s,
        "#   2. removing the entry, and re-running the check to confirm zero regressions."
    );
    let _ = writeln!(s, "schema_version = \"0.3\"");
    let _ = writeln!(s);
    let mut idx = 0usize;
    for ((path, family), count) in &grouped {
        idx += 1;
        let _ = writeln!(s, "[[allow]]");
        let _ = writeln!(s, "id = \"panic-baseline-{idx:04}\"");
        let _ = writeln!(s, "path = \"{path}\"");
        let _ = writeln!(s, "family = \"{family}\"");
        let _ = writeln!(s, "classification = \"baseline\"");
        let _ = writeln!(s, "owner = \"unassigned\"");
        let _ = writeln!(
            s,
            "explanation = \"{count} pre-policy {family} call site(s) at policy adoption; burn down by Q3 2026.\""
        );
        let _ = writeln!(s, "expires = \"2026-09-01\"");
        let _ = writeln!(s);
        let _ = writeln!(s, "[allow.selector]");
        let _ = writeln!(s, "kind = \"baseline_glob\"");
        let _ = writeln!(s, "container = \"*\"");
        let _ = writeln!(s, "callee = \"*\"");
        let _ = writeln!(s);
    }
    std::fs::write(&out_path, &s)?;
    println!(
