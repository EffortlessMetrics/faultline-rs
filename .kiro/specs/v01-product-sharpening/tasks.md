...
This plan implements the v0.1 product sharpening pass across 13 waves organized in three tiers: product differentiation features (Waves 1–6), verification depth (Waves 7–10), and repo maturation (Waves 11–13). Each wave builds on the previous, with checkpoints after major milestones. All code is Rust (stable, edition 2021) targeting the existing hexagonal workspace.

## Tasks

- [ ] 1. Wave 1 — Type changes and contract freeze
  - [ ] 1.1 Add new types to `faultline-types`
  ...
  - [ ] 1.2 Modify existing types in `faultline-types`
  ...
  - [ ] 1.3 Add new `HistoryPort` methods in `faultline-ports`
  ...
  - [ ] 1.4 Update all construction sites and adapters for compilation
...
- [ ] 2. Checkpoint — Wave 1 compilation
...
- [ ] 3. Wave 2 — Ranked suspect surface
  - [ ] 3.1 Implement `SurfaceAnalyzer::rank_suspect_surface` in `faultline-surface`
  ...
  - [ ]* 3.2 Write property test for P43: Suspect surface ranking is sorted and deterministic
  ...
  - [ ]* 3.3 Write property test for P44: Execution surfaces, renames, and deletes score higher
  ...
  - [ ]* 3.4 Write property test for P45: SuspectEntry preserves change_status and surface_kind
  ...
  - [ ]* 3.5 Write property test for P46: SuspectEntry owner_hint matches owners map
  ...
  - [ ] 3.6 Implement CODEOWNERS parsing in `faultline-git`
  ...
  - [ ]* 3.7 Write property test for P55: CODEOWNERS parser determinism
  ...
  - [ ] 3.8 Implement blame frequency in `faultline-git`
  ...
  - [ ] 3.9 Wire suspect surface through `faultline-app`
  ...
  - [ ] 3.10 Render suspect surface in HTML report
...
- [ ] 4. Checkpoint — Wave 2 suspect surface
...
- [ ] 5. Wave 3 — Flake-aware probing
  - [ ] 5.1 Implement `FlakeSignal` computation in `faultline-types`
  ...
  - [ ]* 5.2 Write property test for P48: FlakeSignal stability classification
  ...
  - [ ] 5.3 Add `FlakePolicy` awareness to `LocalizationSession` in `faultline-localization`
  ...
  - [ ]* 5.4 Write property test for P49: Flaky observations degrade confidence
  ...
  - [ ]* 5.5 Write property test for P50: Default FlakePolicy produces no FlakeSignal
  ...
  - [ ] 5.6 Implement flake retry loop in `faultline-app`
  ...
  - [ ] 5.7 Add CLI flags `--retries` and `--stability-threshold` to `faultline-cli`
...
- [ ] 6. Checkpoint — Wave 3 flake-aware probing
  - Run `cargo test -p faultline-types -p faultline-localization -p faultline-app -p faultline-cli`
  - Ensure all tests pass, ask the user if questions arise.

- [ ] 7. Wave 4 — Reproduction capsule
  - [ ] 7.1 Implement `ReproductionCapsule::to_shell_script` in `faultline-types`
  ...
  - [ ]* 7.2 Write property test for P52: Shell script generation contains required fields
  ...
  - [ ] 7.3 Generate reproduction capsules in `faultline-app`
  ...
  - [ ]* 7.4 Write property test for P51: ReproductionCapsule structural correspondence
  ...
  - [ ] 7.5 Add CLI `reproduce` subcommand to `faultline-cli`
...
- [ ] 8. Wave 5 — Run-to-run comparison
  - [ ] 8.1 Implement `compare_runs` pure function in `faultline-types`
  ...
  - [ ]* 8.2 Write property test for P53: compare_runs is total
  ...
  - [ ]* 8.3 Write property test for P54: Self-comparison yields zero diff
  ...
  - [ ] 8.4 Add CLI `diff-runs` subcommand to `faultline-cli`
...
- [ ] 9. Checkpoint — Waves 4–5 capsule and comparison
  - Run `cargo test -p faultline-types -p faultline-app -p faultline-cli`
  - Ensure all tests pass, ask the user if questions arise.

- [ ] 10. Wave 6 — Markdown dossier export
  - [ ] 10.1 Implement `render_markdown` in `faultline-render`
  ...
  - [ ]* 10.2 Write property test for P47: Markdown dossier contains all required sections
  ...
  - [ ] 10.3 Add CLI `--markdown` flag and `export-markdown` subcommand
  ...
  - [ ] 10.4 Update SARIF and JUnit adapters for suspect_surface
...
- [ ] 12. Wave 7 — Mutation coverage expansion
  - [ ] 12.1 Extend `mutants.toml` for all adapter and export crates
  ...
  - [ ] 12.2 Add `--crate` flag to `cargo xtask mutants`
    - Update `crates/xtask/src/lib.rs` (or relevant module) to accept `--crate <name>` for targeted mutation runs
    - _Requirements: 6.3_

- [ ] 13. Wave 8 — Fuzz targets
  - [ ] 13.1 Add fuzz target `fuzz_git_diff_parse`
  ...
  - [ ] 13.2 Add fuzz target `fuzz_store_json`
...
- [ ] 15. Wave 9 — BDD/scenario coverage
  ...
  - [ ] 15.4 Add export surfaces scenario
  ...
  - [ ] 15.5 Add CI contract failure scenario
