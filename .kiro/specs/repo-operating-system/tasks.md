...
- [ ] 1. Wave 1 — Foundation: xtask crate skeleton, workspace deps, mise.toml, Justfile
  - [ ] 1.1 Create `crates/xtask/Cargo.toml` and `crates/xtask/src/main.rs` with clap-derived CLI skeleton
  ...
  - [ ] 1.2 Add workspace dependencies for new crates
  ...
  - [ ] 1.3 Create `.mise.toml` at repo root
    - Pin `rust = "stable"` and `just = "latest"`
    - Set `CARGO_TERM_COLOR = "always"` in `[env]`
    - _Requirements: 5.6_

  - [ ] 1.4 Create `Justfile` at repo root
  ...
  - [ ]* 1.5 Write unit test for xtask help completeness
...
- [ ] 3. Wave 2 — Schema & Evidence: schemars derive, JSON Schema generation, golden tests via insta
  - [ ] 3.1 Add `schemars` derive to `faultline-types`
  ...
  - [ ] 3.2 Create `schemas/` directory and implement schema generation in xtask
  ...
  - [ ] 3.3 Add `insta` golden tests for `analysis.json` and `index.html` in `faultline-render`
  ...
  - [ ] 3.4 Add `insta` golden test for CLI `--help` in `faultline-cli`
  ...
  - [ ]* 3.5 Write property test for JSON Schema validation of reports
  ...
  - [ ]* 3.6 Write property test for schema drift detection
...
- [ ] 5. Wave 3 — Export Adapters: faultline-sarif, faultline-junit crates
  - [ ] 5.1 Create `crates/faultline-sarif/` crate
  ...
  - [ ] 5.2 Create `crates/faultline-junit/` crate
  ...
  - [ ]* 5.3 Write property test for SARIF export structural validity
  ...
  - [ ]* 5.4 Write property test for JUnit XML export structural validity
...
- [ ] 7. Wave 4 — Documentation Layer: pattern catalog, scenario atlas, verification matrix, crate map, ADR template, handbook
  - [ ] 7.1 Create `docs/patterns/catalog.md` with 10 named patterns
  ...
  - [ ] 7.2 Create `docs/adr/TEMPLATE.md`
  ...
  - [ ] 7.3 Create `docs/scenarios/scenario_index.md`
  ...
  - [ ] 7.4 Create `docs/scenarios/behavior_map.md`
  ...
  - [ ] 7.5 Create `docs/verification-matrix.md`
  ...
  - [ ] 7.7 Create `docs/handbook/README.md`
    - Architecture handbook entry linking to Pattern Catalog, ADR index, Scenario Atlas, and Crate Map
    - _Requirements: 1.6_

  - [ ]* 7.8 Write property test for pattern entry structural completeness
  ...
  - [ ]* 7.9 Write property test for scenario entry structural completeness
...
- [ ] 9. Wave 5 — Teaching Layer: AGENTS.md, TESTING.md, RELEASE.md, MAINTAINERS.md
  - [ ] 9.1 Create `AGENTS.md` at repo root
  ...
  - [ ] 9.2 Create `TESTING.md` at repo root
  ...
  - [ ] 9.3 Create `RELEASE.md` at repo root
  ...
  - [ ] 9.4 Create `MAINTAINERS.md` at repo root
...
- [ ] 10. Checkpoint — Wave 5 complete
  - Ensure all four teaching layer files exist at repo root and are cross-referenced. Ask the user if questions arise.

- [ ] 11. Wave 6 — Diátaxis Site: mdBook setup, tutorials, how-to, explanations, reference
  - [ ] 11.1 Create mdBook configuration and directory structure
...
- [ ] 12. Checkpoint — Wave 6 complete
  - Ensure `docs/book/` structure is complete, `SUMMARY.md` links all pages. Ask the user if questions arise.

- [ ] 13. Wave 7 — CI & Governance: CI workflows, deny.toml, contract-aware checks
  - [ ] 13.1 Create `deny.toml` at repo root
  ...
  - [ ] 13.2 Implement xtask `ci-fast` subcommand
  ...
  - [ ] 13.3 Implement xtask `ci-full` subcommand
  ...
  - [ ] 13.4 Implement xtask tool-detection helper
  ...
  - [ ] 13.7 Create `.github/workflows/ci.yml` (ci-fast, every push)
    - Checkout, install Rust stable with rustfmt + clippy, run `cargo xtask ci-fast`
    - _Requirements: 8.6, 8.8_

  - [ ] 13.8 Create `.github/workflows/ci-full.yml` (PRs)
    - Checkout, install Rust stable, install `cargo-insta`, run `cargo xtask ci-full`
    - _Requirements: 8.1, 8.6_

  - [ ] 13.9 Create `.github/workflows/ci-extended.yml` (manual/release)
  ...
  - [ ] 13.10 Create `.github/workflows/release.yml` (tag push)
    - Trigger on tag push `v*`
    - Install governance tools, run `cargo xtask release-check`
    - _Requirements: 9.5_

  - [ ]* 13.11 Write property test for tool detection error messages
  ...
  - [ ]* 13.12 Write property test for CI failure message contract identification
...
- [ ] 15. Wave 8 — Scaffold Commands: scaffold crate, adr, scenario, doc subcommands
  - [ ] 15.1 Implement `cargo xtask scaffold crate <name> --tier <tier>`
  ...
  - [ ] 15.2 Implement `cargo xtask scaffold adr <title>`
  ...
  - [ ] 15.3 Implement `cargo xtask scaffold scenario <name> --crate <crate>`
    - Generate test file stub in target crate's `tests/` directory
    - Add placeholder entry to `docs/scenarios/scenario_index.md`
    - _Requirements: 10.4_

  - [ ] 15.4 Implement `cargo xtask scaffold doc <title> --section <section>`
  ...
  - [ ] 15.5 Implement scaffold input validation
  ...
  - [ ]* 15.6 Write property test for scaffold crate generation
  ...
  - [ ]* 15.7 Write property test for scaffold ADR sequential numbering
  ...
  - [ ]* 15.8 Write property test for scaffold file generation (scenarios and docs)
  ...
  - [ ]* 15.9 Write property test for scaffold input validation
...
- [ ] 17. Wave 9 — Advanced Verification: mutation testing config, fuzz targets
  - [ ] 17.1 Configure mutation testing for `faultline-localization`
  ...
  - [ ] 17.2 Create fuzz target for `AnalysisReport` JSON deserialization
  ...
  - [ ] 17.3 Implement scenario atlas verification in xtask
  ...
  - [ ]* 17.4 Write property test for scenario atlas consistency
...
- [ ] 19. Wave 10 — Final Integration and Wiring
  - [ ] 19.1 Wire export adapters into xtask and CI
  ...
  - [ ] 19.2 Update `docs/scenarios/scenario_index.md` with all new tests
  ...
  - [ ] 19.3 Update `docs/scenarios/behavior_map.md` with new cross-references
    - Add rows for all 10 repo-operating-system requirements
    - Cross-reference to new ADRs, scenarios, fixtures, and artifacts
    - _Requirements: 2.4_

  - [ ] 19.4 Update `docs/crate-map.md` and `docs/verification-matrix.md`
  ...
  - [ ] 19.5 Verify all teaching layer cross-references
