...
- [x] 1. Wave 0 — Freeze Contract: Type Changes, Code Vocabulary, Port Extension
  - [x] 1.1 Add `MaxProbesExhausted` variant to `AmbiguityReason` in `faultline-codes`
  ...
  - [x] 1.2 Remove `edge_refine_threshold` from `SearchPolicy` in `faultline-types`
    - Remove the `edge_refine_threshold` field from `SearchPolicy`
    - Update `Default` impl to only set `max_probes: 64`
    - _Requirements: 3.1_

  - [x] 1.3 Add `schema_version` to `AnalysisReport` in `faultline-types`
    - Add `pub schema_version: String` field to `AnalysisReport`
    - Default value is `"0.1.0"`
    - _Requirements: 1.4, 1.5, 1.6_

  - [x] 1.4 Add diagnostic fields to `ProbeObservation` in `faultline-types`
  ...
  - [x] 1.5 Add version metadata to `RunHandle` in `faultline-types`
  ...
  - [x] 1.6 Add `load_report` to `RunStorePort` in `faultline-ports`
    - Add `fn load_report(&self, run: &RunHandle) -> Result<Option<AnalysisReport>>` to the trait
    - _Requirements: 6.11_

  - [x] 1.7 Fix all compilation errors from type changes across the workspace
...
- [x] 2. Checkpoint — Wave 0 complete (contract frozen)
  - Ensure `cargo test` passes for the entire workspace. Ask the user if questions arise.

- [x] 3. Wave 1 — Prove Core: Localization Engine Semantics
  - [x] 3.1 Implement `MaxProbesExhausted` outcome in `LocalizationSession::outcome()` in `faultline-localization`
  ...
  - [x] 3.2 Update `LocalizationSession` to consume (not assign) sequence indices in `faultline-localization`
  ...
  - [x] 3.3 Remove all references to `edge_refine_threshold` in `faultline-localization`
    - Remove any code paths that reference `policy.edge_refine_threshold`
    - _Requirements: 3.1_

  - [x] 3.4 Review and document `prop_monotonic_window_narrowing` in `faultline-localization`
  ...
  - [x] 3.5 Write property test P25: Max-Probe Exhaustion Produces Explicit Outcome
  ...
  - [x] 3.6 Write property test P26: Observation Sequence Order Preservation
...
- [x] 4. Checkpoint — Wave 1 complete (localization semantics proven)
...
- [x] 5. Wave 2 — Real Fixtures + Adapter Realism: GitRepoBuilder, Git Hardening
  - [x] 5.1 Implement `GitRepoBuilder` in `faultline-fixtures`
  ...
  - [x] 5.2 Add environment validation to `GitAdapter::new()` in `faultline-git`
  ...
  - [x] 5.3 Add stale worktree cleanup to `GitAdapter::new()` in `faultline-git`
  ...
  - [x] 5.4 Harden `cleanup_checkout` resilience in `faultline-git`
  ...
  - [x] 5.5 Add fixture scenario: exact-first-bad-commit (real Git)
  ...
  - [x] 5.6 Add fixture scenario: first-parent-merge-history (real Git)
  ...
  - [x] 5.7 Add fixture scenario: rename-and-delete (real Git)
...
- [x] 6. Checkpoint — Wave 2 complete (real fixtures + adapter realism)
  - Ensure `cargo test -p faultline-fixtures -p faultline-git` passes. Ask the user if questions arise.

- [x] 7. Wave 3 — Safe Resumability: Atomic Writes, Lock File, Store Enhancements
  - [x] 7.1 Implement `atomic_write` helper in `faultline-store`
    - Write to `{target}.tmp` then `fs::rename` to target
    - Replace all `fs::write` calls in `FileRunStore` with `atomic_write`
    - _Requirements: 6.1_

  - [x] 7.2 Implement lock file for single-writer guard in `faultline-store`
  ...
  - [x] 7.6 Implement full-log persistence in `faultline-store`
  ...
  - [x] 7.7 Write property test P29: Store Observation Sequence Order
  ...
  - [x] 7.8 Write property test P30: Version Metadata Persistence
  ...
  - [x] 7.9 Write property test P31: Report Load Round-Trip
  ...
  - [x] 7.10 Write property test P24: Schema Version Round-Trip
  ...
  - [x] 7.11 Add store hardening unit tests
...
- [x] 8. Checkpoint — Wave 3 complete (persistence safe)
  - Ensure `cargo test -p faultline-store` passes. Ask the user if questions arise.

- [x] 9. Wave 4 — Harden Probe Executor
  - [x] 9.1 Implement signal-aware classification in `faultline-probe-exec`
  ...
  - [x] 9.2 Implement output truncation in `faultline-probe-exec`
  ...
  - [x] 9.3 Populate `probe_command` and `working_dir` on `ProbeObservation` in `faultline-probe-exec`
  ...
  - [x] 9.4 Write property test P27: Signal-Aware Exit Code Classification
  ...
  - [x] 9.5 Write property test P28: Observation Structural Completeness (Extended)
  ...
  - [x] 9.6 Add probe executor hardening unit tests
...
- [x] 10. Checkpoint — Wave 4 complete (probe executor hardened)
  - Ensure `cargo test -p faultline-probe-exec` passes. Ask the user if questions arise.

- [x] 11. Wave 5 — Operator Surface: CLI Polish, Exit Codes, HTML Enhancements
  - [x] 11.1 Implement `OperatorCode`-based exit codes in `faultline-cli`
  ...
  - [x] 11.2 Add new CLI flags: `--resume`, `--force`, `--fresh`, `--no-render`, `--shell`, `--env`
  ...
  - [x] 11.3 Enhance CLI terminal summary output
  ...
  - [x] 11.4 Wire `--env` and `--shell` into `ProbeSpec` construction in `faultline-cli`
    - Pass `--env` pairs into `ProbeSpec::Exec.env` or as environment for `ProbeSpec::Shell`
    - Map `--shell` value to `ShellKind` variant
    - _Requirements: 5.5, 5.6, 5.7_

  - [x] 11.5 Enhance HTML report with visual outcome distinction in `faultline-render`
  ...
  - [x] 11.6 Add ambiguity reason badges to HTML report in `faultline-render`
    - Render each `AmbiguityReason` as a `<span class="badge badge-{reason}">` next to the outcome summary
    - _Requirements: 8.9_

  - [x] 11.7 Implement temporal observation timeline in HTML report in `faultline-render`
  ...
  - [x] 11.8 Add execution surface separation in HTML report in `faultline-render`
    - Render `execution_surfaces` in a separate highlighted section from general changed paths
    - _Requirements: 8.11_

  - [x] 11.9 Add log file links in HTML report in `faultline-render`
    - When per-probe log files exist (from output truncation), render relative `<a>` links to them
    - _Requirements: 8.12_

  - [x] 11.10 Write property test P32: OperatorCode Exit Code Mapping
  ...
  - [x] 11.11 Write property test P33: HTML Outcome Visual Distinction and Ambiguity Badges
  ...
  - [x] 11.12 Write property test P34: HTML Temporal Observation Order
  ...
  - [x] 11.13 Write property test P35: HTML Execution Surface Separation
  ...
  - [x] 11.14 Write property test P36: CLI Help Flag Completeness
  ...
  - [x] 11.15 Add CLI unit tests for new flag validation
...
- [x] 12. Checkpoint — Wave 5 complete (operator surface polished)
  - Ensure `cargo test -p faultline-cli -p faultline-render` passes. Ask the user if questions arise.

- [x] 13. Wave 6 — App Orchestrator Updates, Fixture Scenarios, Integration
  - [x] 13.1 Implement `sequence_index` assignment in `FaultlineApp::localize`
  ...
  - [x] 13.2 Update `FaultlineApp::localize` to set `schema_version` on `AnalysisReport`
    - Set `schema_version: "0.1.0".into()` when constructing the report
    - _Requirements: 1.4_

  - [x] 13.3 Update `FaultlineApp::localize` to handle `MaxProbesExhausted`
  ...
  - [x] 13.4 Wire `--force`, `--fresh`, and `--no-render` behavior through `FaultlineApp`
  ...
  - [x] 13.5 Wire full-log persistence through `FaultlineApp::localize`
  ...
  - [x] 13.6 Add fixture scenarios for localization edge cases (skipped, timed-out, non-monotonic)
  ...
  - [x] 13.7 Add fixture scenario: interrupted-run-and-resume
  ...
  - [x] 13.8 Add snapshot tests for `analysis.json` and HTML report
  ...
  - [x] 13.9 Update integration tests in `faultline-app` for new type fields
...
- [x] 14. Checkpoint — Wave 6 complete (app orchestrator updated, fixtures complete)
  - Ensure `cargo test -p faultline-app -p faultline-localization -p faultline-render` passes.
  - HUMAN REVIEW GATE: if any property test failed and was fixed during this wave, the diff must be reviewed before proceeding.
  - Ask the user if questions arise.

- [x] 15. Wave 7 — Dogfood and Release Readiness
  - [x] 15.1 Add CI configuration (`.github/workflows/ci.yml`)
    - GitHub Actions workflow for Linux: `cargo build`, `cargo test`, `cargo fmt --check`, `cargo clippy -- -D warnings`
    - _Requirements: 2.5_

  - [x] 15.2 Add workspace-level smoke test
  ...
  - [x] 15.3 Verify and fix `cargo fmt --check` and `cargo clippy` compliance
    - Run `cargo fmt --check` and fix any violations
    - Run `cargo clippy` and fix any warnings
    - _Requirements: 2.3, 2.4_

  - [x] 15.4 Update BUILDING.md with accurate build prerequisites and commands
    - Document Rust stable requirement, `git` on PATH, build/test/fmt/clippy commands
    - _Requirements: 2.6_

  - [x] 15.5 Verify naming consistency across all artifacts
  ...
  - [x] 15.6 Add CLI `--help` version display from workspace Cargo.toml
    - Add `#[command(version)]` to the `Cli` struct so `--help` and `--version` show the workspace version
    - _Requirements: 1.7_

  - [x] 15.7 Update README with quickstart referencing real artifacts and document packaging
  ...
  - [x] 15.8 Add release workflow or document source-only releases
...
- [x] 16. Final checkpoint — All tests pass, release ready
